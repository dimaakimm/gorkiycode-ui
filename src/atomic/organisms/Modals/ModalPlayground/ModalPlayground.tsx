...
import {
  ModalMap,
  ModalMapProps,
} from "../../../molecules/ModalMap/ModalMap.tsx";
...
const ModalPlayground: React.FC<ModalPlaygroundProps> = (
  props: ModalPlaygroundProps,
) => {
  ...
  return (
    <ModalMap active={active} onClose={onClose}>
      <div
        className={baseStyles.modal_content}
      ...
      >
        ...
        <div className={baseStyles.playground__status}>
          <h2 className={baseStyles.playground__statusHeader}>Состояние</h2>
          <p className={baseStyles.playground__statusValue}>свободно</p>
        </div>
        <LoadChart data={data} />
        ...
        <div>
          <Button className={baseStyles.iWillCumBtn} type={"secondary"}>
            я приду!
          </Button>
        </div>
      </div>
